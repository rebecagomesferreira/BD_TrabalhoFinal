...
-- Para criar usuarios Automaticamente
...
-- Trigger para criar usuário automaticamente ao cadastrar CLIENTE
DELIMITER $$

CREATE TRIGGER trg_criar_usuario_cliente
AFTER INSERT ON CLIENTE
FOR EACH ROW
BEGIN
    INSERT INTO usuarios (nome_usuario, senha, id_cliente, id_grupo)
    VALUES (
        NEW.nome,
        CONCAT('cli', NEW.id_cliente),
        NEW.id_cliente,
        'VIS'  -- grupo visualizador existente
    );
END$$

CREATE TRIGGER trg_criar_usuario_funcionario
AFTER INSERT ON FUNCIONARIO
FOR EACH ROW
BEGIN
    INSERT INTO usuarios (nome_usuario, senha, id_funcionario, id_grupo)
    VALUES (
        NEW.nome,
        CONCAT('op', NEW.id_funcionario),
        NEW.id_funcionario,
        'OPE'  -- grupo operador existente
    );
END$$
