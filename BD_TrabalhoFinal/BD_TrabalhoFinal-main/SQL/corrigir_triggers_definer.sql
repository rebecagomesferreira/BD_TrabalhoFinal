...
-- Exclui triggers antigas (se existirem)
DROP TRIGGER IF EXISTS verifica_espaco_estoque;
DROP TRIGGER IF EXISTS atualiza_nivel_estoque;
DROP TRIGGER IF EXISTS impede_exclusao_fornecedor;
DROP TRIGGER IF EXISTS verifica_preco_minimo;
DROP TRIGGER IF EXISTS verifica_cpf_duplicado_cliente;

-- Trigger 1: verifica espaço no estoque antes de inserir
DELIMITER $$
...
-- Trigger 2: atualiza nível do estoque após inserção
DELIMITER $$
...
-- Trigger 3: impede exclusão de fornecedor com documentos de coleta
DELIMITER $$
...
-- Trigger 4: impede atualização de preço abaixo do mínimo
DELIMITER $$
...
-- Trigger 5: impede cliente duplicado (CPF/CNPJ)
DELIMITER $$
